...
public class Sensor extends LobClass {
     ...
     *
     * @return list of frame processing block
     * \else
     * 创建传感器推荐滤波列表
     *
     * @return 滤波列表
    ...
    public List<Filter> createRecommendedFilters() {
        ...
        long[] filterHandles = nCreateRecommendedFilters(mHandle);
        for (long filterHandle : filterHandles) {
            filters.add(new Filter(filterHandle));
        }
        return filters;
    }
    ...
    private static native long[] nCreateRecommendedFilters(long handle);
