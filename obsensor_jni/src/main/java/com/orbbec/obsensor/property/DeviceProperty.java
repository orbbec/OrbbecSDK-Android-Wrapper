...
public enum DeviceProperty {
     ...
     * Currently using for DabaiA device,if the exposure value is lower than a certain threshold, the laser is turned off;
     * if it exceeds another threshold, the laser is turned on again.
     * \else
     * 低曝光激光控制
     * 当前为达白A设备使用，当曝光值低于某个阈值时，激光关闭；当超过另一个阈值时，激光再次开启。
     * \endif
     */
     ...
     * The Value type is @ref OBRegionOfInterest
     * \else
     * 彩色传感器 AE ROI 配置
     * 值类型为 @ref OBRegionOfInterest
     * \endif
     */
     ...
     * 深度传感器 AE ROI 配置
     * 值类型为 @ref OBRegionOfInterest
     * 由于ir传感器和深度传感器是同一物理传感器，所以这个属性也会影响ir传感器。
     * \endif
     */
     ...
     * \else
     * 设置和获取USB设备帧跳过模式状态，true：帧跳过模式，false：非帧跳过模式
     * \endif
     */
