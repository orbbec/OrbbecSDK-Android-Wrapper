...
inline void ob_handle_error(JNIEnv *env, ob_error *error) {
  ...
  // Handle Error
  std::string message = (char *)ob_error_get_message(error);
  std::string function = (char *)ob_error_get_function(error);
  std::string args = (char *)ob_error_get_args(error);
...
inline void ob_throw_error(JNIEnv *env, const char *function_name,
                           const char *message) {
