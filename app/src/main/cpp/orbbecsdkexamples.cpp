...
void yuyvToRgb(uint8_t *src, uint8_t *dst, int width, int height) {
    uint8_t *srcPtr = src;
    uint8_t *dstPtr = dst;
    int y0, u, y1, v;
    int r, g, b;
...
Java_com_orbbec_orbbecsdkexamples_utils_ImageUtils_nDepthToRgb(JNIEnv *env, jclass clazz,
                                                               jobject srcBuffer,
                                                               jobject dstBuffer) {
    auto *srcPtr = static_cast<uint8_t *>(env->GetDirectBufferAddress(srcBuffer));
    jsize srcSize = env->GetDirectBufferCapacity(srcBuffer);

    auto *dstPtr = static_cast<uint8_t *>(env->GetDirectBufferAddress(dstBuffer));
...
Java_com_orbbec_orbbecsdkexamples_utils_ImageUtils_nDepthAlignToColor(JNIEnv *env, jclass clazz,
                                                                      ...
                                                                      jfloat alpha) {
    auto *colorPtr = static_cast<uint8_t *>(env->GetDirectBufferAddress(colorData));
    auto *depthPtr = static_cast<uint8_t *>(env->GetDirectBufferAddress(depthData));
...
extern "C" JNIEXPORT void JNICALL
Java_com_orbbec_orbbecsdkexamples_utils_ImageUtils_nYuyvToRgb(JNIEnv *env, jclass clazz,
                                                              jobject src_buffer,
                                                              jobject dst_buffer, jint width,
                                                              jint height) {
    auto *srcPtr = static_cast<uint8_t *>(env->GetDirectBufferAddress(src_buffer));
    auto *dstPtr = static_cast<uint8_t *>(env->GetDirectBufferAddress(dst_buffer));
    yuyvToRgb(srcPtr, dstPtr, width, height);
}
...
Java_com_orbbec_orbbecsdkexamples_utils_ImageUtils_nUyvyToRgb(JNIEnv *env, jclass clazz,
                                                              ...
                                                              jint width,
                                                              jint height) {
    auto *srcPtr = reinterpret_cast<uint8_t *>(env->GetDirectBufferAddress(srcBuffer));
    auto *dstPtr = reinterpret_cast<uint8_t *>(env->GetDirectBufferAddress(dstBuffer));
